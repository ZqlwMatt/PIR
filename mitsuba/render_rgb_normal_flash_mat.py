...
scenes = sorted(os.listdir(asset_dir))
for scene in scenes[:]:
    scene = 'camera'
    ...
    for split in ['train', 'test']:
        ...
        for index, img_name in enumerate(img_list[98:99]):
            ...
            cmd = (
                'mitsuba -b 10 rgb_normal_flash_hdr_mat.xml -D fov={} -D width={} -D height={} -D c2w="{}" '
            ...
            output_dir = os.path.join(out_split_dir, 'image')

            """ Disalbe unmute to supress output"""
            parse_cmd = "python parseEXR_edit.py {} --out_dir {} --get_img --get_normal --get_depth --unmute --do_img_gamma".format(out_fpath, output_dir)
            print(parse_cmd)
            os.system(parse_cmd)
