...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        mid_z_vals = z_vals + dists * 0.5 # mid_z_vals.shape == z_vals.shape
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        """
        NeuS: up sampling given a fixed inv_s
        """
        batch_size, n_samples = z_vals.shape
    ...
    def render_core(
        ...
        # idr network (pts, normal, views, f)
        ...
        # Estimate signed distances at section points, assume linear along the normal
        estimated_next_sdf = sdf + iter_cos * dists.reshape(-1, 1) * 0.5
        estimated_prev_sdf = sdf - iter_cos * dists.reshape(-1, 1) * 0.5

        prev_cdf = torch.sigmoid(estimated_prev_sdf * inv_s)
        next_cdf = torch.sigmoid(estimated_next_sdf * inv_s) # Phi_s(x)

        # components of `rho(t)` in paper
        p = prev_cdf - next_cdf
        c = prev_cdf
